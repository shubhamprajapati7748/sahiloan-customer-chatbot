...
class Nodes:
    def __init__(self):
        ...
        self.embeddings = OpenAIEmbeddings(
            model="text-embedding-3-small", dimensions=1024, api_key=settings.OPENAI_API_KEY.get_secret_value()
        )
    ...
    def _load_intent_eval_dataset(self):
        ...
        try:
            ...
            self.eval_embeddings = [self.embeddings.embed_query(query) for query in self.eval_queries]
    ...
    def intent_router(self, state: ChatState) -> ChatState:
        try:
            ...
            messages = [SystemMessage(content=INTENT_ROUTER_SYSTEM_PROMPT), HumanMessage(content=user_input)]
    ...
    def general_agent(self, state: ChatState) -> ChatState:
        ...
        try:
            ...
            if context_chunks:
                ...
                user_message = GENERAL_AGENT_WITH_CONTEXT_TEMPLATE.format(context=context, query=user_query)
            else:
                # No relevant context found - use no-context template
                user_message = GENERAL_AGENT_NO_CONTEXT_TEMPLATE.format(query=user_query)
                logger.warning("No relevant context found in Pinecone")

            # Step 3: Generate response using LLM
            messages = [SystemMessage(content=GENERAL_AGENT_SYSTEM_PROMPT), HumanMessage(content=user_message)]
            ...
            latency = (time.perf_counter() - node_start) * 1000
            logger.info(
                f"general_agent_completed | chunks_used: {len(context_chunks)} | latency: {latency:.2f}ms | llm: {llm_fact['model_name']}"
            )
