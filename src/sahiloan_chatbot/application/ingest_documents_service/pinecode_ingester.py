...
class PineconeIngester:
    ...
    def __init__(self, chunk_size: int = 1000, chunk_overlap: int = 200, batch_size: int = 100):
        """
        ...
        logger.info(f"PineconeIngester initialized | chunk_size: {chunk_size} | overlap: {chunk_overlap}")
    ...
    def chunk_text(self, text: str, metadata: Dict) -> List[Dict]:
        ...
        for i, chunk in enumerate(chunks):
            chunk_metadata = {**metadata, "chunk_index": i, "total_chunks": len(chunks), "chunk_size": len(chunk)}
            chunk_data.append({"text": chunk, "metadata": chunk_metadata})
    ...
    def chunk_faq_by_questions(self, text: str, metadata: Dict) -> List[Dict]:
        ...
        for section in sections:
            ...
            chunk_data.append({"text": chunk_text, "metadata": chunk_metadata})
    ...
    def upsert_chunks(self, chunks: List[Dict]):
        ...
        for chunk in chunks:
            ...
            # Generate unique ID
            chunk_id = self.generate_chunk_id(chunk["metadata"]["source"], chunk["metadata"]["chunk_index"])
    ...
    def ingest_file(self, file_path: Path):
        ...
        # Prepare metadata
        metadata = {"source": str(file_path), "filename": file_path.name, "file_type": "markdown"}
